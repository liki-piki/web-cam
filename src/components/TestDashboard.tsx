              ...
              <div className="space-y-4">
                {sessions.map(session => (
                  <div key={session.studentName} className="p-4 border rounded-lg space-y-2">
                    ...
                    {session.deviceInfo?.detectedDevices && session.deviceInfo.detectedDevices.length > 0 && (
                      <div className="mt-2 text-sm text-muted-foreground">
                        <p className="text-xs font-medium">Detected Devices:</p>
                        <p>{session.deviceInfo.detectedDevices.join(', ')}</p>
                      </div>
                    )}
                    <div className="flex items-center gap-2">
